...
export default function ContentfulProfessionalSerializer(items: ContentfulEntries["content"]["items"]): ProfessionalExperience[] {
  const initialProfessionalExperiences: ProfessionalExperience[] = [];
  const sortedItems = items.map(({ fields }) => ({
  ...
  return sortedItems.reduce((memo, fields, index) => {
    const job = {
      ...
      startDate: formatISO(fields.startDate),
      endDate: formatISO(fields.endDate),
    };
